...
productos = {1: {"nombre": "Manzana", "precio": 1.5, "stock": 100},
...
def obtener_productos(id_producto):
    ...
    def calcular_total(lista_productos):
        """calcular e; total de la compra"""
    for producto in lista_productos:
        total += producto ['precio']
    return total 

def agregar_producto (id_producto, cantidad, lista_productos):
    """Agregar un producto a la lista de compra y regusitrar la venta """ 
    producto= obtener_productos (id_producto)
    if producto:
        lista_productos.append({"nombre": producto["nombre"],"precio": producto["precio"]* cantidad})
        #reguistrar la venta en el historial
        
        historial_ventas.append({"producto":producto["nombre"],"cantidad":cantidad,"precio_total":producto["precio"]*cantidad})

    else:
        raise ValueError(f"Prodcuto con ID {id_producto} no encontrado")
